...
module.exports = {
    register: ( req, res) => {
        ...
        db.add_user([username]).then(usersList => {
            if(usersList.length > 0) {
                ...
                //hash the password
                bcrypt.hash(password,10).then(hash => { //hash takes place of password so it cant be see
                    db.createUser(username, hash, email).then( ()=> {
                        req.session.user = {
                            username: username,
                            email: email,
                        }
    ...
    loginUser: (req, res) => {
        ...
        db.add_user(username).then(user => {
            if(user.length > 0) {
